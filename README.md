...
- 강사님의 소개 부분 들고 왔습니다. 아래 내용을 참고하여 2026년 새해 1월 안에 완강 할 예정입니다.
- `master` 브랜치의 소스코드는 **개정 3판** 소스코드입니다.
- **개정 2판** 소스코드는 `2pan` 브랜치에 있습니다.
- **초판** 소스코드는 `1pan` 브랜치에 있습니다.

## 설치 안내

소스코드를 받으신 후에는 터미널에서 코드의 **루트 경로(`package.json`이 있는 위치)** 로 이동하신 뒤 아래 명령어를 실행해 `node_modules`를 설치하셔야 합니다.

- `npm i`

> `.env` 파일과 `node_modules` 폴더는 이 소스 코드에 포함되어 있지 않습니다.
